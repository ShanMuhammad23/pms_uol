...
/** Calendar days in a standard year — used for applicable duration factor. */
...
/**
 * Tenure vs financial-year end (FY ends 30 Jun):
 * - >= 12 months → Fully Eligible
 * - DOJ on/after 1 Apr of the FY end year → Not Eligible
 * - >= 3 and < 12 months (and before 1 Apr) → Partially Eligible
...
export function computeAppraisalEligibility(dateOfJoining, options = {}) {
  ...
  const { cycleStart, cycleEnd } = deriveUolFiscalYearWindow(referenceEndYear);
  // Joining on/after 1 Apr of the FY end year is too late for that cycle.
  const partialEligibilityCutoff = new Date(referenceEndYear, 3, 1);
  ...
  // Inclusive month math treats 1 Apr–30 Jun as 3 months; business rule is
  // that anyone joining on/after 1 Apr of the FY end year is Not Eligible.
  if (
    doj >= partialEligibilityCutoff ||
    applicableMonths < minimumPartialMonths
  ) {
    return {
      status: "Not Eligible",
      uolExperienceYears,
