...
interface UsersTableColumnHeaderFilterProps {
  column: UsersTableColumnDef;
  users: UserRecord[];
  allUsers?: UserRecord[];
  filters: UsersMasterFilterState;
  onTextChange: (columnId: UsersMasterFilterTextColumnId, next: string) => void;
  onMultiChange: (
...
export function UsersTableColumnHeaderFilter({
  ...
  const isText = isUsersMasterFilterTextColumn(column.id);
  const active = isUsersColumnFilterActive(filters, column.id);
  ...
  const applyTextFilter = () => {
    onTextChange(column.id as UsersMasterFilterTextColumnId, draftText);
    setOpen(false);
  };
