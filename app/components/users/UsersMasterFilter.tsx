...
interface UsersMasterFilterProps {
  users: UserRecord[];
  allUsers?: UserRecord[];
...
export function UsersMasterFilter({
  ...
  const activeCount = countActiveUsersMasterFilters(filters);
          ...
          >
            <div className="space-y-5 border-t border-slate-200/70 px-5 pb-5 pt-4 dark:border-white/5">
              <div className="grid gap-x-4 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                {USERS_MASTER_FILTER_TEXT_COLUMNS.map((column) => (
                  <TextFilterField
                    key={column.id}
                    label={column.label}
                    value={
                    ...
                    onChange={(next) =>
                      onTextChange(
                        column.id as UsersMasterFilterTextColumnId,
                        next,
                      )
                    }
                  />
              ...
              <div className="grid gap-x-4 gap-y-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {USERS_MASTER_FILTER_MULTI_COLUMNS.map((column) => {
