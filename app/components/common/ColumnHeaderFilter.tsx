...
interface ColumnHeaderFilterProps {
  ...
  onMultiChange?: (next: string[] | null) => void;
}
...
export function ColumnHeaderFilter({
  ...
  const draftSyncKey = open && isText ? `${label}:${textValue}` : "";
            ...
            {isText ? (
              <div className="space-y-2 p-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-slate-200 bg-slate-50 py-1.5 pl-8 pr-3 text-xs text-slate-700 outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 dark:border-white/10 dark:bg-slate-950 dark:text-slate-300"
                  />
                </div>
                <button
                  type="button"
                  onClick={applyTextFilter}
                  className="w-full rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-primary/90"
                >
                  Apply
                </button>
              </div>
              ...
              <>
                <div className="flex shrink-0 items-center justify-between gap-2 border-b border-slate-100 px-3 py-1.5 dark:border-white/5">
                  <button
                    type="button"
                    onClick={() => onMultiChange?.(allValues)}
                      ...
                      return (
                        <li key={option.value}>
                          <button
                            ...
                            className="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs text-slate-700 hover:bg-primary/5 dark:text-slate-300 dark:hover:bg-white/4"
                          >
                            <span
                              className={cn(
