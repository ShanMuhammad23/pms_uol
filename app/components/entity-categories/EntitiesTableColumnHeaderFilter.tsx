...
import {
  ...
  isEntitiesMasterFilterTextColumn,
...
import type {
  EntitiesTableColumnDef,
  EntitiesTableColumnId,
...
interface EntitiesTableColumnHeaderFilterProps {
  column: EntitiesTableColumnDef;
  entities: EntityRecord[];
  filters: EntitiesMasterFilterState;
  onTextChange: (
    columnId: EntitiesMasterFilterTextColumnId,
    next: string,
  ) => void;
  onMultiChange: (
...
export function EntitiesTableColumnHeaderFilter({
  column,
  entities,
  filters,
  onTextChange,
  onMultiChange,
  onNumericChange,
}: EntitiesTableColumnHeaderFilterProps) {
  ...
  const isText = isEntitiesMasterFilterTextColumn(column.id);
  const active = isEntitiesColumnFilterActive(filters, column.id);
  ...
  const options = useMemo(() => {
    ...
    return buildEntitiesMasterFilterOptions(
      entities,
      column,
      filters,
      selectedValues,
  ...
  const applyTextFilter = () => {
    onTextChange(column.id as EntitiesMasterFilterTextColumnId, draftText);
    setOpen(false);
  };
  ...
  const clearFilter = () => {
    if (isText) {
      setDraftText("");
      onTextChange(column.id as EntitiesMasterFilterTextColumnId, "");
    } else {
      onMultiChange(column.id, null);
    }
