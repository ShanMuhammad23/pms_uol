...
export async function POST(request: Request) {
  ...
  try {
    ...
    const created = await createIncrementMatrix({
      financialYearId,
    ...
    console.error("Failed to create increment matrix:", error);
    return NextResponse.json(
      { error: "Failed to create increment matrix." },
      { status: 500 },
    );
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const updated = await updateIncrementMatrixIdentity({
      financialYearId,
      matrixLabel: body.matrixLabel,
    ...
    console.error("Failed to update increment matrix:", error);
    return NextResponse.json(
      { error: "Failed to update increment matrix." },
      { status: 500 },
    );
  }
}
