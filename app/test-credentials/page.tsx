...
import { signInWithTestCredentials } from "@/lib/queries/auth-client";

/**
 * Hidden test route for email/password login.
 *
 * This page is NOT linked anywhere in the navigation. It must be accessed by
 * typing the URL directly: /test-credentials
 *
 * The test-credentials provider only exists outside production (or when
 * ALLOW_TEST_LOGIN=true is set — see auth.ts), so this page will fail with
 * "CredentialsSignin" in production builds without that flag.
 */

interface TestCredentialsMessage {
  tone: "success" | "error";
  text: string;
}

export default function TestCredentialsLoginPage() {
  const router = useRouter();
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [message, setMessage] = useState<TestCredentialsMessage | null>(null);

  const mutation = useMutation({
    mutationFn: ({ email, password }: { email: string; password: string }) =>
      signInWithTestCredentials(email, password),
    onSuccess: (response) => {
      setMessage({
        tone: "success",
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!password) {
      setMessage({ tone: "error", text: "Please enter a password." });
      return;
    }
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="space-y-2 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Sign in with an employee email and password for testing.
            <br />
        ...
        >
          <div className="space-y-2">
            <label
              htmlFor="test-credentials-email"
          ...
          <div className="space-y-2">
            <label
              htmlFor="test-credentials-password"
              className="text-sm font-medium text-slate-700 dark:text-slate-300"
            >
              Password
            </label>
