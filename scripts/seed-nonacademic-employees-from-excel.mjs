/**
 * Seed non-academic staff from Excel.
 *
 * Maps:
 *   SAP Code              → users.employee_id
 *   Name                  → first_name / last_name
 *   Email IDs             → email (fallback: {sap}@uol.edu.pk)
 *   DOJ                   → date_of_joining
 ...
 * Legacy NOT NULL enums are set to SUPPORT_STAFF / PROFESSIONAL on insert only.
 *
 * Usage:
 *   node scripts/seed-nonacademic-employees-from-excel.mjs
 *   node scripts/seed-nonacademic-employees-from-excel.mjs --dry-run
 *   node scripts/seed-nonacademic-employees-from-excel.mjs --file "path/to/file.xlsx"
 */
...
const DEFAULT_EXCEL_FILE = join(
  rootDir,
  "public",
  "Non-Academic Staff - Performance Portal with Email IDs.xlsx",
);
const DEFAULT_PASSWORD = "Employee@123";
const NON_ACADEMIC_C1_NAME = "Non-Academic";
const PLACEHOLDER_EMP_CATEGORY = "SUPPORT_STAFF";
const PLACEHOLDER_EMP_SUB_CATEGORY = "PROFESSIONAL";
...
function formatDateOfJoining(value) {
  ...
  // Excel often stores short dates like 6/8/26
...
function pickBestEntityMatch(matches) {
  ...
  // Prefer more specific units (C2/C3) over umbrella C1 when names collide.
...
function readRowsFromExcel(excelPath, sheetName) {
  ...
  for (let index = 0; index < rawRows.length; index += 1) {
    ...
    const position = normalizeText(row.Position) || null;
    const rawEmail = normalizeText(row["Email IDs"]).toLowerCase();
    ...
    if (seenEmail.has(email)) {
      entries.push({
        ...
        sapCode,
        email,
        error: `Duplicate email (also on row ${seenEmail.get(email)} for SAP ${
          entries.find((e) => e.excelRow === seenEmail.get(email))?.sapCode ?? "?"
        })`,
      });
      continue;
    }
...
async function resolveEntityMap(client, organizationalUnits, categoryIds, dryRun) {
  ...
  if (needsCreate.length === 0) {
    return { resolved, summary, nonAcademicC1Id };
  }
...
async function upsertUser(client, payload, hasRoleCategory) {
  ...
  // Explicitly keep staff categories null on insert; do not overwrite on update.
...
async function main() {
  ...
  if (args.help) {
    console.log(`Seed non-academic employees from Excel.
  ...
  try {
    ...
    for (const entry of validEntries) {
      try {
        ...
        // Guard: do not let this email collide with a different employee_id.
        ...
        const result = await upsertUser(
          client,
          {
            ...
            systemRole: "EMPLOYEE",
            empCategory: PLACEHOLDER_EMP_CATEGORY,
            empSubCategory: PLACEHOLDER_EMP_SUB_CATEGORY,
        ...
        if (result.inserted) {
          summary.inserted += 1;
          console.log(
            `  inserted: SAP ${entry.sapCode} → entity ${entityInfo.matchedName}`,
          );
        } else {
          summary.updated += 1;
          console.log(
            `  updated: SAP ${entry.sapCode} → entity ${entityInfo.matchedName}`,
          );
        }
      } catch (error) {
        summary.errors += 1;
    ...
    console.log(
      "  staff_category_id / staff_sub_category_id: left NULL for all seeded rows",
    );
  } catch (error) {
    if (!args.dryRun) {
